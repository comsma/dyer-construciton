            ...
            <nav className="bg-gray-800 border-b border-gray-700">
                    ...
                    <div className="flex justify-between h-16">
                        <div className="flex">
                            ...
                            <div className="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                                ...
                                {user.has_admin || user.has_modify_documents?
                                    <NavLink href={route('jobs.list')} active={route().current('jobs.list')}>
                                        Jobs
                                    </NavLink> : null
                                }
                                {user.has_admin || user.has_view_documents?
                                    <NavLink href={route('plan.list')} active={route().current('plan.list')}>
                                        My Plans
                                    </NavLink> : null
                                }
                ...
                <div className={(showingNavigationDropdown ? 'block' : 'hidden') + ' sm:hidden'}>
                    <div className="pt-2 pb-3 space-y-1">
                        ...
                        {user.has_admin || user.has_modify_documents?
                            <ResponsiveNavLink href={route('jobs.list')} active={route().current('jobs.list')}>
                                Jobs
                            </ResponsiveNavLink> :null
                        }
                        {user.has_admin || user.has_view_documents?
                            <ResponsiveNavLink href={route('plan.list')} active={route().current('plan.list')}>
                                My Plans
                            </ResponsiveNavLink> :null
                        }
